...
https://github.com/XingangPan/IBN-Net/blob/master/ibnnet/densenet_ibn.py
"""
...
class _DenseLayer_(nn.Sequential):
    ...
    def forward(self, x):
        new_features = super(_DenseLayer_, self).forward(x)
        if self.drop_rate > 0:
            new_features = F.dropout(new_features, p=self.drop_rate, training=self.training)
        return torch.cat([x, new_features], 1)


class _DenseBlock_(nn.Sequential):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, ibn):
        super(_DenseBlock_, self).__init__()
        for i in range(num_layers):
            if ibn and i % 3 == 0:
                layer = _DenseLayer_(num_input_features + i * growth_rate, growth_rate, bn_size, drop_rate, True)
            else:
                layer = _DenseLayer_(num_input_features + i * growth_rate, growth_rate, bn_size, drop_rate, False)
            self.add_module('denselayer%d' % (i + 1), layer)


class _Transition_(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        super(_Transition_, self).__init__()
...
class DenseNet_IBN(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i >= 3:
                ibn = False
            block = _DenseBlock_(num_layers=num_layers, num_input_features=num_features,
                                bn_size=bn_size, growth_rate=growth_rate, drop_rate=drop_rate, ibn=ibn)
            self.features.add_module('denseblock%d' % (i + 1), block)
            num_features = num_features + num_layers * growth_rate
            if i != len(block_config) - 1:
                trans = _Transition_(num_input_features=num_features, num_output_features=num_features // 2)
